...
void* perform_tosses(void* rank) {
    long long int tosses_in_circle = 0;
    unsigned int seed = time(nullptr) + (long)rank;  // unique seed for each thread
    ...
    cout<<"Thread "<<(long)rank<<" tosses_in_circle: "<<tosses_in_circle<<endl;
